...
1. Determine the choreo_id — build on what the user already stated; if not clear, ask which choreo they want to create or edit
   ...
   - If the user has no idea (no choreo name mentioned), set choreo_id to null and mode to `create` — it will be determined during requirements gathering, and must follow the `<verb>_*` format (e.g. `review_code`, `edit_choreo`)
