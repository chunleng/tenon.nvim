...
1. Determine what user wants, from:
  - First round (no `goal` in choreo memory): what user specified before the choreo starts
  - Returning round (`goal` in choreo memory, after implementation confirmed): the `goal` belongs to the previous cycle, ignore it
