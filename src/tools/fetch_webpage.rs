...
async fn answer_with_prompt(markdown: &str, prompt: &str) -> Result<String, ToolError> {
    ...
    let worker = SimpleTenonWorkerAgent::new(
        ...
        "Use only the webpage content. If the prompt cannot be answered from the content, say \"Not found in page.\" Do not infer or fabricate. Webpage content only. No preamble/hedge/commentary/source refs. Preserve format: code→code blocks, steps→numbered lists, comparisons→tables, items→bullets",
        true,
    )
    .map_err(|e| {
